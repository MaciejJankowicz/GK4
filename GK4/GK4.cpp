// GK4.cpp : Defines the entry point for the console application.
//
...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("blonde_girl_white_skin_3.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_girl_white_skin_3.tga");
		exit(0);
	}
	...
	error = load_targa("blonde_girl_white_skin_3_gray.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku blonde_girl_white_skin_3_gray.tga");
		exit(0);
	}
