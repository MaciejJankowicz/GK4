// GK4_2.cpp : Defines the entry point for the console application.
//
...
void DisplayScene()
{
	...
	// rysowanie pięciu drzew
	glBindTexture(GL_TEXTURE_2D, TREE);
	glPushMatrix();
	glTranslatef(-1.25f, 0.0, 0.0);
...
void GenerateTextures()
{
	...
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku grass_clean.tga");
		exit(0);
	}
	...
	error = load_targa("oak_tree_big.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku oak_tree_big.tga");
		exit(0);
	}
